...
@step('we get the projects via the API')
def create_project_api(context):
    assert context.persona
    client = context.persona['client']
    url = '/api/v2/projects'
...
@step('we create a project called "{project_name}" via the API')
...
@step('we get the volumes via the API')
