...
def calculate_correction():
    ...
    start_date = '2016-09-01T05:00+00:00'
    #end_date = '2016-10-17T00:00+00:00'
...
def calculate_correction_for(allocation_source, start_date, end_date, tacc_username_list):
    ...
    for user in allocation_source.all_users:
        ...
        if username not in user_delta_cache:
            user_delta_cache[username] = create_report(start_date,end_date,user_id=username)
        compute_used_atmo = get_allocation_source_usage_for(user_delta_cache[username],allocation_source.name) #total_usage(
            #username,
    ...
    #    print 'total_used Atmosphere for %s = ' %(username), float(compute_used_atmo)
        ...
        correction_delta.append((
            ...
            delta,
            usage_not_reported))
        pprint(correction_delta[-1])
    print 'END calculate_correction_for: %s' % allocation_source.name
    return correction_delta


def get_allocation_source_usage_for(user_data,allocation_source_name):
    total_allocation = 0
...
def calculate_total_allocation_for_user(username, allocation_source):
    ...
    reports = TASAllocationReport.objects.filter(resource_name="Jetstream").filter(
        username=username).filter(project_name=allocation_source.name).filter(start_date__gte='2016-09-01T05:00+00:00')
    for report in reports:
        if not report.success:
            usage_not_reported += report.compute_used
        total_used += report.compute_used
    #print 'total_used Jetstream for %s = '%(username), total_used
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 1 and sys.argv[1] == '--create':
        ...
        delta = calculate_correction()
        with open('correction_deltas.p','w+') as f:
            cPickle.dump(delta,f)
